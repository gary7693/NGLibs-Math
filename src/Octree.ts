...
export class Octree<T> {
    ...
    boundary: Vector3[];
    level: number;
    constructor(bound: Vector3[], level: number = 3) {
        this.level = level;
        this.boundary = bound.splice(0);
        let { boundary } = this;
        if (level <= 0) return this;
        this.child = [];
        let center = Vector3.averageList(boundary);
        for (let i = 0; i < 2; i++) {
            for (let j = 0; j < 2; j++) {
                for (let k = 0; k < 2; k++) {
                    let b0 = new Vector3(
                        i ? boundary[0].x : boundary[1].x,
                        j ? boundary[0].y : boundary[1].y,
                        k ? boundary[0].z : boundary[1].z
                    );
    ...
    isCloseTo(pos: Vector3, range: number = 0): boolean {
        let { boundary: boundry } = this;
    ...
    nearBy(pos: Vector3, range: number, res: OctTreeElement<T>[], filter: (T) => boolean = (check) => true): boolean {
     ...
     * @returns Result
     */
    nearest(pos: Vector3, res: { curNearest?: OctTreeElement<T>, distance: number }, filter: (T) => boolean = (check) => true): boolean {
        let { contents, child, level } = this;
        if (!contents || contents.length == 0)
            return false;
        if (res.curNearest) {
